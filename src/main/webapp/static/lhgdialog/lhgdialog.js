...
lhgdialog.fn =
{
	...
	show: function()
	{
		...
		!arguments[0] && this._lock && $('#lockMask' + this.config.id,_doc).showIt();
		return this;
	},
	...
	hide: function()
	{
		...
		!arguments[0] && this._lock && $('#lockMask' + this.config.id,_doc).hideIt();
		return this;
	},
...
//		}
		...
		//如果弹出窗口对象中没有窗口对象属性，移除顶层框架的lock_scroll属性
		if(top.lhgdialog && top.lhgdialog.list){
			var hasProp = false;
		    for(var currObj in top.lhgdialog.list){
		    	hasProp = true;
		    	break;
		    }
		    if(!hasProp) {
	...
	lock: function()
	{
		var that = this, frm,
			...
			config = that.config,
			mask = $('#lockMask' + that.config.id,_doc)[0] || null,
		...
		if( !mask )
		{
			...
			style = mask.style;
			mask.id = 'lockMask' + that.config.id;
...
//			if( _ie6 ) mask.innerHTML = frm;
	...
	unlock: function()
	{
		var that = this,
		    config = that.config,
			mask = $('#lockMask' + that.config.id,_doc)[0];
		if( mask && that._lock )
		{
		    // 无限级锁屏
...
//			}
	...
	maxSize: function()
	{
		...
		if(!window.event){//如果没有事件触发，为用户自动调用，默认设置为需要最大化
...
//		    !that._lock && _$html.removeClass('ui_lock_scroll');
	...
	_iframe: function( url )
	{
		...
		$iframe = $('<iframe  id="PopDialog' + config.id + '" name="PopDialog' + config.id + '" frameborder="0" src="" ' +
		...
		var load = that._fmLoad = function()
		{
			...
			try{
				...
				that.dgWin = iwin;
				that.dgDoc = that.dgWin.document;
		...
		// 绑定iframe元素lhgDG属性为窗口自身对象，在内容页中此属性很重要
		that.iframe.lhgDG = that;
	},
	...
	_reset: function( test )
	{
		...
		if(test)
		{
			//IE6下遮罩大小改变
			if( that._lock && _ie6 )
			    $('#lockMask'+ that.config.id,_doc).css({ width:tw + 'px', height:tt + 'px' });
	...
	_addEvent: function()
	{
		...
		// 双击标题栏最大化还窗口事件
		if( config.max )
		    DOM.title.bind('dblclick',function(){ that.maxSize();return false; });
	},
...
//	$('div[id^=lockMask]',_doc)[0] && $('div[id^=lockMask]',_doc).remove();
